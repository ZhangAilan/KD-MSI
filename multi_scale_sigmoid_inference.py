...
def get_cam(ori_imageA,ori_imageB, scale):
        ...
        # postprocessing
        cams = F.sigmoid(features)
        cams = (cams[0] + cams[1].flip(-1))/2
...
parser.add_argument('--student_combination', default='minus', type=str)
###############################################################################
...
if __name__ == '__main__':
    ...
    # Network
    ###################################################################################
    model = Classifier_Siamese(args.architecture, 1, args.mode, args.student_combination)
    model = model.cuda()
    model.eval()
