...
if %START_STEP% LEQ 2 (
    ...
    python multi_scale_sigmoid_inference.py --data_dir %DATA_DIR% --tag WHU_KD_T_minus_S_cat --student_combination cat --scales 0.5,1.0,1.25,2.0
)

if %START_STEP% LEQ 3 (
    echo [Step 3] Generate pseudo labels
    python make_pseudo_labels.py --data_dir %DATA_DIR% --experiment_name WHU_KD_T_minus_S_cat@train@scale=0.5,1.0,1.25,2.0
)

if %START_STEP% LEQ 4 (
    echo [Step 4] Train change detection model
    python train_change_detection.py --data_dir %DATA_DIR% --tag WHU_weakly_change_detection --label_name WHU_KD_T_minus_S_cat@train@scale=0.5,1.0,1.25,2.0@crf=0@255 --fully_supervised False
)
